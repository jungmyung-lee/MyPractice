...
import { auth, storage } from "../config/firebaseConfig"; // storage도 export 했는지 확인!
import MultiToggleButtonGroup from "../beta_app/MultiToggleButtonGroup";

const OnSite2: React.FC = () => {
  ...
  if (isFormComplete()) {
    ...
    try {
      ...
      const downloadUrl = await uploadBase64ToStorage(
        data.file!,
        `quoteImages/${uid}_${timestamp}.jpg`
      );
    ...
    <IonPage>
          <div className="tab-container" >
              ...
              <div style={{padding: '0px 16px',display: 'flex', flexDirection: 'column', gap: '8px' }} className="header">
                  <Title fontSize={28} content="출장 수리 (2/3)" />
