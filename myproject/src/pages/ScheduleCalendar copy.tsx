...
const ScheduleCalendar: React.FC = () => {
    ...
    <IonPage>
      <IonContent className="schedule-page">
        <div style={{ textAlign: "left", marginBottom: "12px" }}>
          <h1 className="calendar-title">정비소 일정 관리</h1>
      ...
      >
        ...
        {selectedDay && (
          <div className="day-schedule-section">
            <h3>{selectedDay}. {"일월화수목금토"[new Date(selectedYear, selectedMonth - 1, selectedDay).getDay()]}</h3>
  ...
  <p><strong>{item.start.slice(11, 16)}~{item.end.slice(11, 16)}</strong> {item.title}</p>
